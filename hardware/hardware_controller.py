...
class HardwareController:
    def __init__(self, gps: UBX):
        ...
        self.__steer_adc_measurements = deque(maxlen=3)
        self._steering_changed = False

        # Steering PID Controller
        # Not implemented
        self.P_gain_steer = 0.5
        self.D_gain_steer = 0.5
    ...
    def _update_current_steering_angle(self) -> None:
        self.__steer_adc_measurements.append(self.steer_adc.voltage)
        pot_voltage_avg = sum(self.__steer_adc_measurements)/len(self.__steer_adc_measurements)
        lat_distance = _convert_voltage_to_distance(pot_voltage_avg)
        self._current_steering_angle = round(np.rad2deg(_convert_distance_to_angle(lat_distance)), 1)

    def _increment_steering_output_voltage(self, new_voltage: float):
        v = max(new_voltage, self._min_steer_voltage)
        self._steer_output_voltage = min(v, self._max_steer_voltage)
